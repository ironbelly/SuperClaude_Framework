# TASK LIST GENERATOR FROM ROADMAP (Deterministic, Value‑Preserving) v2.1
...
- **Steps:** 3–8 numbered imperative steps.
...
### 5.3 Traceability Matrix (mandatory, minimal)
Add a Traceability Matrix section that connects:
- `R-###` (Roadmap Item IDs) → `T<PP>.<TT>` (Tasks) → `D-####` (Deliverables) → intended artifact paths.
...
- 6–10 bullets summarizing rules you applied (phase renumbering, task ID scheme, checkpoint cadence, clarification task rule, deliverable registry, effort/risk mappings, traceability matrix).
...
| Deliverable ID | Task ID | Roadmap Item ID(s) | Deliverable (short) | Intended Artifact Paths | Effort | Risk |
|---:|---:|---:|---|---|---|---|
...
**Notes:** <optional; max 2 lines; include deterministic tie-breaker outcomes if needed>
...
| Timestamp (ISO 8601) | Task ID | Deliverable ID(s) | Action Taken (≤ 12 words) | Validation Run (verbatim cmd or "Manual") | Result (Pass/Fail/TBD) | Evidence Path |
|---|---:|---:|---|---|---|---|
